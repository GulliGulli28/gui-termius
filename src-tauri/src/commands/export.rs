...
#[tauri::command]
pub fn export_workspace(state: State<'_, AppState>, path: String) -> Result<(), String> {
    let workspace = state.workspace.lock().expect("lock poisoned").clone();
    let data = ex::make_workspace_export(&workspace);
...
pub fn export_host(
    ...
    let workspace = state.workspace.lock().expect("lock poisoned").clone();
    let data = ex::make_host_export(&workspace, host_id)
