// Adapted from iced_term's backend.rs (MIT, Copyright (c) 2024 Ilia Shvyrialkin,
// see ../LICENSE-iced_term). The rendering-facing API (`Backend`, `Command`,
// `RenderableContent`, `TerminalSize`, `LinkAction`, `MouseButton`) is kept
// identical so `view.rs` and `terminal.rs` are unmodified. What changed is the
// data source: instead of owning a local PTY (`alacritty_terminal::tty` +
// `EventLoop`), this backend is fed by an arbitrary byte transport (an SSH
// shell channel, in this workspace) through plain channels, so the widget has
// no idea whether it is driving a local shell or a remote one.
...
use alacritty_terminal::term::{self, cell::Cell, test::TermSize, viewport_to_point, Term, TermMode};
...
/// Output of the terminal widget: bytes to write to the transport, or a
/// request to change the remote window size.
#[derive(Debug, Clone)]
...
pub struct Backend {
    ...
    size: TerminalSize,
    input: mpsc::Sender<TermCommand>,
    last_content: RenderableContent,
    pub(crate) url_regex: RegexSearch,
}

impl Backend {
    pub fn new(_id: u64, pty_event_proxy_sender: mpsc::Sender<Event>, settings: BackendSettings) -> Result<Self> {
        let config = term::Config::default();
        let terminal_size = TerminalSize::default();
        let event_proxy = EventProxy(pty_event_proxy_sender.clone());
        ...
        spawn_ingest_loop(term.clone(), settings.output, pty_event_proxy_sender);
        ...
        match cmd {
            Command::ProcessAlacrittyEvent(event) => {
                match event {
                    ...
                    Event::PtyWrite(data) => self.write(data.into_bytes()),
                    _ => {},
                };
            },
            Command::Write(input) => {
    ...
    fn process_link_action(&mut self, terminal: &Term<EventProxy>, link_action: LinkAction, point: Point) {
        match link_action {
            LinkAction::Hover => {
                self.last_content.hovered_hyperlink = self.regex_match_at(terminal, point, &mut self.url_regex.clone());
            },
            LinkAction::Clear => {
    ...
    fn process_mouse_report(&self, button: MouseButton, modifiers: Modifiers, point: Point, pressed: bool) {
        let mut mods = 0;
    ...
    fn start_selection(&mut self, terminal: &mut Term<EventProxy>, selection_type: SelectionType, x: f32, y: f32) {
        let location = Self::selection_point(x, y, &self.size, terminal.grid().display_offset());
        terminal.selection = Some(Selection::new(selection_type, location, self.selection_side(x)));
    }
    ...
    pub fn selection_point(x: f32, y: f32, terminal_size: &TerminalSize, display_offset: usize) -> Point {
    ...
    fn resize(&mut self, terminal: &mut Term<EventProxy>, layout_size: Option<Size<f32>>, font_measure: Option<Size<f32>>) {
        ...
        if lines > 0 && cols > 0 {
            ...
            let _ = self.input.try_send(TermCommand::Resize { cols, rows: lines });
            terminal.resize(TermSize::new(self.size.num_cols as usize, self.size.num_lines as usize));
        }
    }

    fn write(&self, input: Vec<u8>) {
        let _ = self.input.try_send(TermCommand::Write(input));
    }

    fn scroll(&mut self, terminal: &mut Term<EventProxy>, delta_value: i32) {
        if delta_value != 0 {
            let scroll = Scroll::Delta(delta_value);
            if terminal.mode().contains(TermMode::ALTERNATE_SCROLL | TermMode::ALT_SCREEN) {
    ...
    fn regex_match_at(&self, terminal: &Term<EventProxy>, point: Point, regex: &mut RegexSearch) -> Option<Match> {
        visible_regex_match_iter(terminal, regex).find(|rm| rm.contains(&point))
    }
}

/// Reads bytes coming from the transport (e.g. an SSH shell channel) and feeds
/// them into the terminal's VTE parser, waking the widget up to redraw after
/// each chunk — this replaces alacritty's own PTY-reading `EventLoop`.
fn spawn_ingest_loop(term: Arc<FairMutex<Term<EventProxy>>>, mut output: mpsc::Receiver<Vec<u8>>, wakeup: mpsc::Sender<Event>) {
    tokio::spawn(async move {
        ...
        while let Some(bytes) = output.recv().await {
            ...
            if wakeup.try_send(Event::Wakeup).is_err() {
                break;
            }
        }
        let _ = wakeup.try_send(Event::Exit);
    });
}

/// Copied from alacritty/src/display/hint.rs: iterate over all visible regex matches.
fn visible_regex_match_iter<'a>(term: &'a Term<EventProxy>, regex: &'a mut RegexSearch) -> impl Iterator<Item = Match> + 'a {
...
impl EventListener for EventProxy {
    fn send_event(&self, event: Event) {
        // `try_send` (not `blocking_send`): this can be invoked synchronously
        // from inside the tokio task that drives `spawn_ingest_loop`, where
        // blocking the executor thread is not allowed.
